...
public interface AppointmentJpaRepository extends JpaRepository<AppointmentEntity, Integer> {
    ...
    @Query(value = "SELECT a FROM AppointmentEntity a WHERE a.pet.owner.id = :ownerId ORDER BY a.startDateTime DESC",
           countQuery = "SELECT COUNT(a) FROM AppointmentEntity a WHERE a.pet.owner.id = :ownerId")
